...
do
{
    ...
    switch (opc)
    {
        ...
        case 2:
            ...
            var lineas = System.IO.File.ReadAllLines("C:\archivo\articulos.txt");
            System.Collections.Generic.List<string> nuevoArchivo = new List<string>();

            for (int i = 0; i < usuarios.Count; i++)
            {
                Usuario obj = usuarios[i];
                for (int j = 0; j < obj.articulos.Count; j++)
                {
                    Articulo art = obj.articulos[j];




                    nuevoArchivo.Add(art.getCodigo() + " " + art.getNombre() + ".");


                }
            }
            File.WriteAllLines("C:\archivo\articulos.txt", nuevoArchivo);
